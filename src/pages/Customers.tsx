...
function Customers() {
  ...
  if (error) {
    return <div className="text-red-600">Failed to load customers: {error}</div>;
  }
    ...
    <div>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="mx-4 max-w-md rounded-lg bg-white p-6 shadow-lg w-full">
            ...
            {addError && <div className="mt-3 text-sm text-red-600">{addError}</div>}
      ...
      {filteredCustomers.length === 0 ? (
        ...
        <ul className="divide-y divide-slate-200 rounded-md border border-slate-200 bg-white">
          {filteredCustomers.map((c) => (
            <li
              key={c.id}
              className="space-y-2 p-4 transition hover:bg-slate-50 sm:flex sm:items-center sm:justify-between sm:space-y-0"
